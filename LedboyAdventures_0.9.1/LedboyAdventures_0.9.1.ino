...
#define LEDON PORTA.OUT &= ~PIN3_bm;// led ON
...
void setup() {
  ...
  if (readSupplyVoltage() < 2400) goToSleep(); // To preserve the LIC to stay above 2.2v no operation is allowed under 2.4v

  while (readSupplyVoltage() > 3800) LEDON; // If voltage is above 3.8v the led is on to increase Amp consumption and lower batt.
...
void intro(void) {// intro screen.
  ...
  while (BUTTONHIGH) {
    ...
    if ((interruptTimer - timer) > 150) {
      ...
      soildState = !soildState;
      uint8_t voltageReading = map(readSupplyVoltage(), 2700, 3800, 10, 115);
...
void buttonDebounce(void) {
  timer = interruptTimer;
  while (BUTTONLOW || (interruptTimer - timer) < 200);
}
