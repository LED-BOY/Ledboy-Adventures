...
void setup() {
  ...
  TCA0.SINGLE.PER   = 0;                   // Count all the way up to 0x00FF (255) - 8-bit PWM. At 20MHz, this gives ~78.125kHz PWM
  TCA0.SINGLE.CMP0  = 254;
  ...
  for (uint8_t x = 2; x < 40; x += 36) { // led PWM and led blinking
    while (BUTTONLOW);
    __asm__("nop\n\t");
    ...
    while (BUTTONHIGH) {
      ...
      if (interruptTimer % x == 0) {
        PORTA.OUT |= PIN3_bm;// led on
      } else {
        PORTA.OUT &= ~PIN3_bm;// led off
      }
    }
  }
  ...
  //_PROTECTED_WRITE(WDT.CTRLA, WDT_PERIOD_8KCLK_gc); // no window, 8 seconds // no window, 8 seconds); //enable the WDT
  oled.begin();// start oled screen
  interruptTimer = 0;
  while (interruptTimer < 500);// wait 500 millis
}
...
void game() {
  ...
  uint8_t enemiesPos [3] = {50, 80, 120};
  uint8_t score = 0;
  ...
  while (lives > 0) {
    ...
    if ((interruptTimer - timer) > 50) { // updates every 50 milliseconds
      ...
      for (uint8_t x = 0; x < 127; x += 8) { // trees and soild draw
...
void intro(void) {// intro screen.
  ...
  //drawTitle(30, 1, titleImage, 0, 74);
  ...
  while (BUTTONHIGH) {
    ...
    if ((interruptTimer - timer) > 150) {
      ...
      soildState = !soildState;
      uint8_t voltageReading = map(readSupplyVoltage(), 2950, 3850, 10, 115);
      oled.drawLine(10, 0, voltageReading, 0b00111111);
      drawSprite(0, 0, battIcon, 0);
      timer = interruptTimer;
    }

    /*if (interruptTimer > 6000) {
      clearScreen();
...
void drawLedBoy (uint8_t column, uint8_t page, uint8_t sprite[], uint8_t bitShift, bool scrollUp) {
